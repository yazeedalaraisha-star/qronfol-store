const I18N = {
  ar: {
    lang: 'ar', dir: 'rtl', label: 'العربية',
    navProducts: 'المنتجات',
    navAbout: 'عن قرنفل',
    navContact: 'اتصل بنا',
    heroTitle: 'خيطٌ من تراث ..<br>وقلبٌ مع غزة',
    heroDesc: 'من الأردن إلى العالم، نقدم لك قطعًا تراثية تحكي قصص فلسطين',
    heroBtn: 'تسوق الآن',
    searchPlaceholder: 'ابحث عن منتج...',
    sortDefault: 'ترتيب: الافتراضي',
    sortAsc: 'السعر: من الأقل للأعلى',
    sortDesc: 'السعر: من الأعلى للأقل',
    cartEmpty: 'عربة التسوق فارغة',
    cartEmptySub: 'أضف منتجات لتتمكن من الطلب',
    total: 'المجموع',
    checkout: 'إتمام الطلب عبر واتساب',
    sectionProducts: 'منتجاتنا',
    sectionProductsSub: 'كل قطعة تحكي قصة تراث وتمسك بالأرد',
    sectionAbout: 'عن قرنفل',
    aboutCard1Title: 'تراث أصيل',
    aboutCard1Desc: 'نستلهم من تراثنا الفلسطيني العريق، ونقدمه بأيدي ماهرة تحافظ على هوية كل قطعة',
    aboutCard2Title: 'قلب مع غزة',
    aboutCard2Desc: 'جزء من أرباحنا يدعم صمود أهلنا في غزة، لأن القضية ما زالت حية في قلوبنا',
    aboutCard3Title: 'صنع في الأردن',
    aboutCard3Desc: 'ننتج ونشحن من الأردن، ونسعى لنشر التراث الفلسطيني في كل مكان',
    aboutMore: 'اقرأ قصتنا كاملة',
    footerTagline: 'خيطٌ من تراث .. وقلبٌ مع غزة 💚🇵🇸',
    contactTitle: 'تواصل معنا',
    contactSub: 'للطلبات والاستفسارات',
    noProducts: 'لا توجد منتجات مطابقة',
    addToCart: 'أضف إلى السلة',
    removeWishlist: 'إزالة من المفضلة',
    addWishlist: 'أضف إلى المفضلة',
    showWishlist: 'عرض المفضلة',
    outOfStock: 'نفذ من المخزون',
    storyBtn: 'القصة',
    relatedTitle: 'منتجات قد تعجبك',
    treeTitle: '🌿 غابتنا تنمو',
    treeDesc: 'كل طلب يزرع شجرة زيتون في غابتنا الرقمية',
    cartTitle: 'عربة التسوق',
    search: 'بحث',
    cancel: 'إلغاء',
    save: 'حفظ',
    delete: 'حذف',
    edit: 'تعديل',
    confirm: 'تأكيد',
    relatedSub: 'اقتراحات من نفس التصنيف',
    addedToCart: 'تمت الإضافة إلى السلة!',
    addToCartWishlist: 'تمت الإضافة إلى المفضلة',
    removeFromWishlist: 'تمت الإزالة من المفضلة',
    sending: 'جاري الإرسال...',
    submitOrder: 'إرسال الطلب',
    invalidCode: 'كود غير صالح',
    connectionError: 'خطأ في الاتصال',
    emptyCartCheckout: 'عربة التسوق فارغة! أضف منتجات أولاً',
    discountApplied: 'خصم {amount} JOD',
    aboutStory1: 'قرنفل صفحة بلشت لتعبر عن حبي للتطريز الفلسطيني وإيماني بإنه الخيط مش بس هوية .. هو كمان ممكن يكون وسيلة دعم وأمل.',
    aboutStory2: 'بقّدم قطع تطريز فلسطيني يدوي لتكون حاضرة معكم، ورح أخصص جزء من الأرباح لدعم أهل غزة ومساعدتهم في مواجهة برد الشتاء وإعادة الإعمار.',
    aboutStory3: 'بشرائكم قطعة تطريز .. أنتم تشاركون في الحفاظ على التراث ومساندة أهلنا وبناء بلدنا اللي بإذن الله رح نرجع عليها منتصرين.',
    aboutStoryThanks: 'شكراً إنكم معنا 💚',
    aboutInstagram: '@qronfol_by_razan',
    announcementText: '🇵🇸 خيطٌ من تراث .. وقلبٌ مع غزة 💚',
    announcementDelivery: 'التوصيل لكل الأردن 🇯🇴',
    checkoutTitle: 'إتمام الطلب',
    orderSummary: 'ملخص الطلب',
    subtotal: 'المجموع الفرعي',
    shippingLabel: 'التوصيل',
    discountLabel: 'الخصم',
    totalCheckout: 'الإجمالي',
    applyBtn: 'تطبيق',
    couponPlaceholder: 'كود خصم',
    nameLabel: 'الاسم الكامل *',
    phoneLabel: 'رقم الهاتف *',
    addressLabel: 'العنوان *',
    paymentLabel: 'طريقة الدفع',
    notesLabel: 'ملاحظات',
    namePlaceholder: 'مثال: رزان أحمد',
    phonePlaceholder: 'مثال: 0792067277',
    addressPlaceholder: 'المدينة، المنطقة، الشارع، تفاصيل إضافية',
    notesPlaceholder: 'أي ملاحظات إضافية للطلب',
    paymentWhatsapp: 'الدفع عبر واتساب',
    paymentCod: 'الدفع عند الاستلام',
    freeShipping: 'مجاني 🎉',
    catAll: 'الكل',
    catAccessories: 'إكسسوارات',
    catEmbroidery: 'تطريز',
    catDecor: 'ديكور',
    catApparel: 'أزياء',
    badgeNew: 'جديد',
    badgeSale: 'تخفيض',
    badgeBest: 'الأكثر مبيعاً',
    tagGift: '🎁 هدية',
    tagSale: '🔥 مخفض',
    tagFreeShipping: '🚚 توصيل مجاني',
    tagLimited: '⏳ كمية محدودة',
    tagHandmade: '✋ صنع يدوي',
  },
  en: {
    lang: 'en', dir: 'ltr', label: 'English',
    navProducts: 'Products',
    navAbout: 'About Qronfol',
    navContact: 'Contact Us',
    heroTitle: 'A Thread of Heritage..<br>A Heart with Gaza',
    heroDesc: 'From Jordan to the world, we offer heritage pieces that tell Palestine\'s stories',
    heroBtn: 'Shop Now',
    searchPlaceholder: 'Search products...',
    sortDefault: 'Sort: Default',
    sortAsc: 'Price: Low to High',
    sortDesc: 'Price: High to Low',
    cartEmpty: 'Your cart is empty',
    cartEmptySub: 'Add products to start shopping',
    total: 'Total',
    checkout: 'Checkout via WhatsApp',
    sectionProducts: 'Our Products',
    sectionProductsSub: 'Each piece tells a story of heritage',
    sectionAbout: 'About Qronfol',
    aboutCard1Title: 'Authentic Heritage',
    aboutCard1Desc: 'We draw inspiration from our rich Palestinian heritage, crafted by skilled hands preserving each piece\'s identity',
    aboutCard2Title: 'Heart with Gaza',
    aboutCard2Desc: 'Part of our profits supports our people in Gaza, because the cause still lives in our hearts',
    aboutCard3Title: 'Made in Jordan',
    aboutCard3Desc: 'We produce and ship from Jordan, spreading Palestinian heritage everywhere',
    aboutMore: 'Read our full story',
    footerTagline: 'A Thread of Heritage.. A Heart with Gaza 💚🇵🇸',
    contactTitle: 'Contact Us',
    contactSub: 'For orders and inquiries',
    noProducts: 'No matching products found',
    addToCart: 'Add to cart',
    removeWishlist: 'Remove from wishlist',
    addWishlist: 'Add to wishlist',
    showWishlist: 'Show wishlist',
    outOfStock: 'Out of stock',
    storyBtn: 'Story',
    relatedTitle: 'You may also like',
    treeTitle: '🌿 Our Forest Grows',
    treeDesc: 'Every order plants an olive tree in our digital forest',
    cartTitle: 'Shopping Cart',
    search: 'Search',
    cancel: 'Cancel',
    save: 'Save',
    delete: 'Delete',
    edit: 'Edit',
    confirm: 'Confirm',
    relatedSub: 'Suggestions from same category',
    addedToCart: 'Added to cart!',
    addToCartWishlist: 'Added to wishlist',
    removeFromWishlist: 'Removed from wishlist',
    sending: 'Sending...',
    submitOrder: 'Submit Order',
    invalidCode: 'Invalid coupon code',
    connectionError: 'Connection error',
    emptyCartCheckout: 'Cart is empty! Add products first',
    discountApplied: 'Discount {amount} JOD',
    aboutStory1: 'Qronfol started to express my love for Palestinian embroidery, believing that thread is not just identity — it can also be a means of support and hope.',
    aboutStory2: 'I offer handmade Palestinian embroidery pieces to be with you, and I will dedicate part of the profits to support the people of Gaza in facing the winter cold and rebuilding.',
    aboutStory3: 'By purchasing an embroidery piece, you help preserve heritage, support our people, and rebuild our land that we will return to — victorious, God willing.',
    aboutStoryThanks: 'Thank you for being with us 💚',
    aboutInstagram: '@qronfol_by_razan',
    announcementText: '🇵🇸 A Thread of Heritage.. A Heart with Gaza 💚',
    announcementDelivery: 'Delivery across Jordan 🇯🇴',
    checkoutTitle: 'Complete Order',
    orderSummary: 'Order Summary',
    subtotal: 'Subtotal',
    shippingLabel: 'Shipping',
    discountLabel: 'Discount',
    totalCheckout: 'Total',
    applyBtn: 'Apply',
    couponPlaceholder: 'Coupon Code',
    nameLabel: 'Full Name *',
    phoneLabel: 'Phone Number *',
    addressLabel: 'Address *',
    paymentLabel: 'Payment Method',
    notesLabel: 'Notes',
    namePlaceholder: 'e.g. Razan Ahmad',
    phonePlaceholder: 'e.g. 0792067277',
    addressPlaceholder: 'City, Area, Street, Details',
    notesPlaceholder: 'Any additional order notes',
    paymentWhatsapp: 'Pay via WhatsApp',
    paymentCod: 'Cash on Delivery',
    freeShipping: 'Free 🎉',
    catAll: 'All',
    catAccessories: 'Accessories',
    catEmbroidery: 'Embroidery',
    catDecor: 'Decor',
    catApparel: 'Apparel',
    badgeNew: 'New',
    badgeSale: 'Sale',
    badgeBest: 'Best Seller',
    tagGift: '🎁 Gift',
    tagSale: '🔥 Sale',
    tagFreeShipping: '🚚 Free Shipping',
    tagLimited: '⏳ Limited',
    tagHandmade: '✋ Handmade',
  }
};

let currentLang = localStorage.getItem('qronfol_lang') || 'ar';

function getText(key) {
  return I18N[currentLang] && I18N[currentLang][key] !== undefined ? I18N[currentLang][key] : I18N['ar'][key] || key;
}

function switchLanguage(lang) {
  if (!I18N[lang]) return;
  currentLang = lang;
  localStorage.setItem('qronfol_lang', lang);
  document.documentElement.lang = lang;
  document.documentElement.dir = I18N[lang].dir;
  document.querySelectorAll('[data-i18n]').forEach(el => {
    const key = el.dataset.i18n;
    el.innerHTML = getText(key);
  });
  document.querySelectorAll('[data-i18n-placeholder]').forEach(el => {
    el.placeholder = getText(el.dataset.i18nPlaceholder);
  });
  const toggleBtn = document.getElementById('langToggle');
  if (toggleBtn) toggleBtn.textContent = currentLang === 'ar' ? 'EN' : 'AR';
  if (window.renderProducts) window.renderProducts();
}

function initI18n() {
  document.addEventListener('DOMContentLoaded', () => {
    const toggleBtn = document.getElementById('langToggle');
    if (toggleBtn) {
      toggleBtn.addEventListener('click', () => {
        switchLanguage(currentLang === 'ar' ? 'en' : 'ar');
      });
    }
    switchLanguage(currentLang);
  });
}

initI18n();
